...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { LegalesignSdkInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw LegalesignSdkInvalidDataException("`$name` is not set")
            is JsonNull -> throw LegalesignSdkInvalidDataException("`$name` is null")
            else -> throw LegalesignSdkInvalidDataException("`$name` is invalid, received $this")
        }
