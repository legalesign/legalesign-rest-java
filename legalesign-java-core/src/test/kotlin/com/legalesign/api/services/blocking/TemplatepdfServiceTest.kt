// File generated from our OpenAPI spec by Stainless.

package com.legalesign.api.services.blocking

import com.legalesign.api.TestServerExtension
import com.legalesign.api.client.okhttp.LegalesignOkHttpClient
import com.legalesign.api.models.templatepdf.TemplatepdfCreateParams
import com.legalesign.api.models.templatepdf.TemplatepdfListParams
import org.junit.jupiter.api.Disabled
import org.junit.jupiter.api.Test
import org.junit.jupiter.api.extension.ExtendWith

@ExtendWith(TestServerExtension::class)
internal class TemplatepdfServiceTest {

    @Disabled("Prism tests are disabled")
    @Test
    fun create() {
        val client =
            LegalesignOkHttpClient.builder()
                .baseUrl(TestServerExtension.BASE_URL)
                .apiKey("My API Key")
                .build()
        val templatepdfService = client.templatepdf()

        templatepdfService.create(
            TemplatepdfCreateParams.builder()
                .group("/api/v1/group/IK-GV--w1tvt/")
                .pdfFile("U3RhaW5sZXNzIHJvY2tz")
                .archiveUponSend(true)
                .processTags(true)
                .title("title")
                .user("user")
                .build()
        )
    }

    @Disabled("Prism tests are disabled")
    @Test
    fun retrieve() {
        val client =
            LegalesignOkHttpClient.builder()
                .baseUrl(TestServerExtension.BASE_URL)
                .apiKey("My API Key")
                .build()
        val templatepdfService = client.templatepdf()

        val templatePdf = templatepdfService.retrieve("182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e")

        templatePdf.validate()
    }

    @Disabled("Prism tests are disabled")
    @Test
    fun list() {
        val client =
            LegalesignOkHttpClient.builder()
                .baseUrl(TestServerExtension.BASE_URL)
                .apiKey("My API Key")
                .build()
        val templatepdfService = client.templatepdf()

        val templatepdfs =
            templatepdfService.list(
                TemplatepdfListParams.builder()
                    .archive("archive")
                    .group("group")
                    .limit(0L)
                    .offset(0L)
                    .build()
            )

        templatepdfs.validate()
    }

    @Disabled("Prism tests are disabled")
    @Test
    fun archive() {
        val client =
            LegalesignOkHttpClient.builder()
                .baseUrl(TestServerExtension.BASE_URL)
                .apiKey("My API Key")
                .build()
        val templatepdfService = client.templatepdf()

        templatepdfService.archive("pdfId")
    }

    @Disabled("Prism tests are disabled")
    @Test
    fun convertTags() {
        val client =
            LegalesignOkHttpClient.builder()
                .baseUrl(TestServerExtension.BASE_URL)
                .apiKey("My API Key")
                .build()
        val templatepdfService = client.templatepdf()

        templatepdfService.convertTags("pdfId")
    }

    @Disabled("Prism tests are disabled")
    @Test
    fun getEditLink() {
        val client =
            LegalesignOkHttpClient.builder()
                .baseUrl(TestServerExtension.BASE_URL)
                .apiKey("My API Key")
                .build()
        val templatepdfService = client.templatepdf()

        templatepdfService.getEditLink("182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e")
    }
}
