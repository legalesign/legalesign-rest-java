...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { LegalesignInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw LegalesignInvalidDataException("`$name` is not set")
            is JsonNull -> throw LegalesignInvalidDataException("`$name` is null")
            else -> throw LegalesignInvalidDataException("`$name` is invalid, received $this")
        }
